    ...
    switch (item.type) {
      case 'image':
        return (
          <Container>
            <Image
              className={`${style.content__image}` + `${index === 0 ? ` ${style.content__first_type}` : ''}` + `${count - 1 === index ? ` ${style.content__last_type}` : ''}`}
              src={`https:` + item.url}
              ...
              unoptimized="true"
            />
            <span className={style.content__image__description}>{item.description}</span>
          </Container>
        );
