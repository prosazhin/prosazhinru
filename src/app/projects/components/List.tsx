  ...
  return (
    <ul className='grid w-full grid-flow-dense grid-cols-6 gap-24'>
      {firstProjects.map(({ title, description, resourceLinks, tags }, index) => (
        <li
          key={index}
          className='desktop:px-80 desktop:py-64 group rounded-16 bg-basic-lightest hover:bg-primary-lighter print:border-secondary-lighter relative col-span-6 flex flex-col gap-y-24 overflow-hidden px-24 pt-20 pb-24 transition-colors duration-150 print:border'
        >
      ...
      {sortedProjects.map(({ size, title, description, accent, resourceLinks, tags }, index) => (
        <li
          ...
          className={clsx(
            ...
            sizes[size],
            accent
              ? 'desktop:px-40 desktop:pt-32 desktop:pb-40 bg-basic-lightest hover:bg-primary-lighter print:border-secondary-lighter px-24 pt-20 pb-24 print:border'
