...
export default function AboutPage({ page, navigations, contacts, skills }) {
          ...
          {skills
            ...
            .map((skill) => (
              <section className={style.section} key={skill.id}>
                ...
                <p className={style.section__description}>{skill.description}</p>
                {skill.tools && (
                  <React.Fragment>
                    <h5 className={`${style.section__description} ${style.section__description_title}`}>Инструменты</h5>
                    <p className={style.section__description}>{skill.tools}</p>
                  </React.Fragment>
                )}
              </section>
            ))}
