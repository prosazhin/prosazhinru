...
export async function getServerSideProps(context) {
	...
	const contacts = method.contacts.serializer(
		await method.contacts.getList()
	);
	...
	const projects = method.projects.serializer(
		await method.projects.getList()
	);
	...
	yearsList.forEach((year) => {
		year.job = {
			hired:
				jobs.filter(
					(item) =>
						dayjs(item.recruited).format("YYYY") ===
						year.titleString
				)[0] || false,
			fired:
				jobs.filter(
					(item) =>
						dayjs(item.dismissal).format("YYYY") ===
						year.titleString
				)[0] || false,
			work:
				jobs.filter((item) => workNow(item, year.titleString))[0] ||
				false,
		};
